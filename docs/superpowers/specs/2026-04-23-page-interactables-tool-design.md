...
5. `src/ui/tools/page-automation/interactable-support.ts` 使用 `dom-accessibility-api` 的 `computeAccessibleName()` 计算控件名称，并结合原生标签、`role`、`placeholder`、状态属性、几何信息与有界图标语义推断生成候选。
...
10. `src/ui/tools/page-automation/rich-editor-bridge-main.ts` 作为 `MAIN` world content script 注册窄桥接，只在宿主页面暴露可用富代码编辑器 API 时接收 isolated content script 的 DOM 事件请求，并执行编辑器模型写入。
...
interface PageInteractablesSnapshot {
  ...
  items: Array<[
    ...
    name: string,       // 模型友好的控件名称，优先来自 accessible name，也可来自有界 fallback
...
- 对非原生点击容器，允许扫描常见承载元素（例如 `div`、`span`、`li`、`td`、`th`），但必须继续依赖点击线索过滤，例如 `onclick`、`aria-controls`、`aria-haspopup`、`aria-expanded`、非负 `tabindex`、计算后的 `cursor:pointer`，或元素自身携带可识别的短语义 token（例如 `data-icon="external-link"`、`data-testid="help-circle"`）。
- 对常见富代码编辑器容器（例如 `monaco-editor`、`cm-editor`、`CodeMirror`、`ace_editor`），允许将外层可见编辑器区域识别为 `textbox`，并用 `meta.t="code"` 标记。
- 对 Monaco 等编辑器内部的展示层节点（例如 `view-lines`、`view-line`、`monaco-mouse-cursor-text`，通常带有 `role="presentation"` 或 `aria-hidden="true"`），必须归一到外层编辑器 surface；与代码编辑器 surface 高度重叠的辅助可访问性文本框（例如名称包含 `Editor content` 或 `Accessibility Options`）也应被视为同一编辑器的重复候选并移除。快照不应把这些展示层或辅助输入节点暴露为独立控件，动作执行时应先触发目标点击并优先写入点击后产生的真实聚焦元素，再回退到 ref 对应的内部可写目标。
...
- 名称必须优先来自 accessible name 计算；对于无原生语义但有点击线索的容器，允许使用可见文本作为短名称 fallback。对于无文本图标按钮，允许从元素及少量后代的 `aria-label`、`title`、`alt`、`data-icon`、`data-testid`、`id`、`class`、`svg title`、`use[href]` 等短 token 中推断常见动作语义，例如 `like`、`dislike`、`comments`、`bookmark`、`favorite`、`share`、`open`、`help`、`copy`、`menu`、`previous`、`next`。如果同一按钮同时包含图标语义与短可见计数或标签，名称应合并为 `like 32.2K`、`comments 922` 这类更完整的控件语义。如果仍无法得到语义，必须输出 `unlabeled <role>`，并可附加相邻兄弟元素的短文本上下文，例如 `unlabeled button near 32.2K`；不得根据页面坐标、元素序号或特定站点硬编码含义。允许使用 `placeholder`、`title`、当前值作为辅助 `meta.h`，但不能用整页 DOM 文本替代结构化输出。
- 对父子嵌套候选，如果父候选已经覆盖同一语义区域（同 role 且名称相同、包含子名称或子名称为空），必须优先保留父候选，减少重复 ref；命名父候选下的 `unlabeled <role>` 子候选，以及命名 `button` / `link` 父候选下的小型语义图标子候选，也应视为重复装饰目标并移除。
...
- 当前实现不使用 `debugger` 权限。
- content script 已随扩展注入目标页面，工具通过既有 runtime message 链路触发 DOM 侧只读快照与动作执行。
- 富代码编辑器模型写入桥接运行在 `MAIN` world，仅通过 DOM 事件接收当前动作文本，不输出页面对象、源码 HTML 或编辑器内部结构。
...
- DOM 动作目标解析、状态遥测、滚动执行与鼠标事件几何放在 `src/ui/tools/page-automation/action-support.ts`，避免动作入口重新成为混合职责文件。
- 文本写入统一入口与 writer 选择放在 `src/ui/tools/page-automation/text-writer.ts`，按富代码编辑器桥接、浏览器原生编辑命令、DOM fallback 的顺序处理 `page_type` 写入语义。
...
- 富代码编辑器 `MAIN` world 窄桥接放在 `src/ui/tools/page-automation/rich-editor-bridge-main.ts`，避免 isolated content script 直接假设可访问页面 JavaScript 对象；该文件只允许处理富代码编辑器模型写入，不承载通用页面脚本注入能力。
...
- `page_type`
  ...
  - 行为：先对目标 ref 执行点击聚焦，再优先写入当前页面的深层 `activeElement`；如果聚焦元素不是可输入控件，则回退到 ref 对应的内部可写目标。若目标属于富代码编辑器，优先通过 `rich-editor-bridge-main.ts` 在页面 `MAIN` world 调用编辑器模型 API 执行整体替换或追加；桥接不可用时，`clear=true` 先执行选择全部内容的通用键盘信号与浏览器原生 `delete` 命令，再尝试 `insertText`；失败后再回退为派发 `beforeinput`、`paste`、`input`、`change` 等通用文本插入信号并直接更新 DOM 值；返回输入前后的 `value` 或可编辑文本状态，供模型判断写入是否生效。若 ref 指向复合编辑器容器或表单包装行，content 侧应跳过隐藏的占位输入与非文本输入。
- `page_scroll`
